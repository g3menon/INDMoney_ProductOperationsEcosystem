"""Automated eval runner (Phase 1-2 suites)."""
...
from app.evals.pulse_checks import Phase2EvalReport, run_phase2_evals
...
async def run_evals(body: EvalRunRequest | None = None) -> APIEnvelope[dict[str, Any]]:
    ...
    if body.suite == "phase2":
        report: Phase2EvalReport = run_phase2_evals()
        passed = report.score >= 85.0
        return APIEnvelope(
            success=passed,
            message="eval_complete" if passed else "eval_below_threshold",
